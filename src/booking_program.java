...
public class booking_program {
    ...
    private static final int year_today = now.getYear();
    private static final int month_today = now.getMonthValue();
    private static final int day_today = now.getDayOfMonth();

    private static final int BRANCH_NUMBER = 6; //지점은 최대 6개 생성 가능
    private static final int SPACE_NUMBER = 5; //스터디 공간은 최대 5개 생성 가능

    public static void main(String[] args) throws IOException {
        System.out.println("<<스터디 공간 예약 프로그램입니다>>");
        while (true){ //프로그램 종료 전까지 무제한 반복
            System.out.println();
            System.out.println("입장할 모드를 선택해주세요.");
            System.out.println("1)관리자 모드\t\t2)사용자 모드\t\t3)프로그램 종료");
            char mode = checkInput(0);
            if (mode=='1'){
                new Administrator();                //1)관리자 모드
            } else if (mode=='2') {
                new User();                         //2)사용자 모드
            } else if (mode=='3' || mode=='q') {    //프로그램 종료
    ...
    private static class Administrator{
        ...
        class branch_management{
                    ...
                    if (mode == '1') {
                        ...
                        while (branch == '0') {
                            ...
                            if (!branch_exist(str)) {
                                ...
                                System.out.print("※스터디 공간 수정을 원하시면 지점 생성 후 다시 시도해주세요.\n");
                                continue;
                            }
                            branch = str.charAt(0);
                        }
                        if (branch != '0') modify(branch);
                    }
                    else if (mode == '2') {
                        char branch = '0';
                        String[] range = new String[]{"1", "2", "3", "4", "5", "6"};
                        while (branch == '0') {
                            ...
                            if (branch_exist(str)) {
                                System.out.printf("%s번 지점은 이미 존재합니다. (입력을 중단하고 홈 화면으로 돌아가시려면 q를 입력해주세요)\n", str);
                                continue;
                            }
                            branch = str.charAt(0);
                        }
                        ...
                        while (branch == '0') {
                            ...
                            if (!branch_exist(str)) {
                                System.out.printf("%s번 지점은 존재하지 않습니다. (입력을 중단하고 홈 화면으로 돌아가시려면 q를 입력해주세요)\n", str);
                                continue;
                            }
                            if (!can_delete(str)) {
            ...
            //스터디 공간 관리 클래스로 전환
            private void modify(char branch) throws IOException {
                new study_space_management(branch);
            }
        }

        private boolean branch_exist(String str) throws IOException { //지점이 존재하면 true, 존재하지 않으면 false return
            String filePath = "src/Administrator_info.txt";
        ...
        class study_space_management{
            public study_space_management(char branch) throws IOException {
                while (true) {
                    ...
                    if (!space_info.equals("00000")) {
                        ...
                        System.out.println("스터디 공간이 없습니다. 새로 추가해주세요.");
                        System.out.println();
                    }
                    ...
                    if (mode == '1') {
                        if (is_there_any_room) {
                            ...
                            while (space == "0") {
                                ...
                                if (!checkSpace(space_info, str)) {
                                    System.out.printf("※%s호실은 존재하지 않습니다. (인원 수정을 원하시면 호실 생성 후 다시 시도해주세요)\n", str);
                                    continue;
                                }
                                space = str;
                            }
                        ...
                        if (!is_it_full) {
                            ...
                            while (space == "0") {
                                ...
                                if (checkSpace(space_info, str)) {
                                    System.out.printf("※%s호실은 이미 존재합니다. (입력을 중단하고 홈 화면으로 돌아가시려면 q를 입력해주세요)\n", str);
                                    continue;
                                }
                                space = str;
                            }
            ...
            static String read(char branch) throws IOException { //입력한 지점의 스터디 공간 정보 return
                String filePath = "src/Administrator_info.txt";
                File file = new File(filePath); // File 객체 생성
                BufferedReader reader = new BufferedReader(new FileReader(file));
                String line;
    ...
    private static class User{
        ...
        public User() throws IOException { //로그인 과정
            ...
            if (!id.isEmpty()){ //quit이 아니면 예약 모드 입장
                user = id;
                new Reservation();
            }
        }
        ...
        class Reservation{
            public Reservation() throws IOException {
                while (true) {
                    ...
                    System.out.println("1)스터디 공간 조회\t\t2)신규 예약\t\t3)예약 관리");
                    System.out.println("(로그아웃하고 초기 화면으로 돌아가시려면 q를 입력해주세요)");

                    char mode = checkInput(1); //1, 2, 3, q 中 하나 입력받음
                    if (mode == 'q') break;
                        ...
                        while (space == '0') {
                            ...
                            for (int i = 0; i < SPACE_NUMBER; i++) {
                                if (result.charAt(i) == str.charAt(0)) {
                                    space = str.charAt(0);
                                    people_limit = Integer.parseInt(String.valueOf(space_info.charAt(i)), 16);
                                    break;
                                }
                            }
                        ...
                        if (space != '0' && people_limit != 0) {
                            ...
                            while (people == 0) {
                                ...
                                try {
                                    ...
                                    System.out.printf("최대 %d명 까지만 이용이 가능합니다.\n", people_limit);
                                    continue;
                                }
                                if (Integer.parseInt(str)<=people_limit && Integer.parseInt(str)>0) people = Integer.parseInt(str);
                                else System.out.printf("최대 %d명 까지만 이용이 가능합니다.\n", people_limit);
                    ...
                    if (!file.exists()) {           // 파일이 존재하지 않으면
                        ...
                        while ((line = reader.readLine()) != null) {
                            ...
                            if (!checkDatePassed(year, month, day)) { //미래의 예약만 출력
                                ...
                                System.out.printf("예약장소 : 지점[%s] %s호실\n", branch_data, space_data);
                                System.out.printf("예약시간 : %d시~%d시\n", start, end);
                        ...
                        if (is_there_any_data) {
                            ...
                            while (num == 0) {
                                //System.out.println();
                                System.out.print("[예약변경]변경하고자 하는 예약 넘버를 입력해주세요 >> ");
                                ...
                                if (Integer.parseInt(str) < i && Integer.parseInt(str) > 0) {
                                    ...
                                    delete_date(date_data, branch_data, space_data, start, end);

                                    if (!execute('2')) { //변경 취소이므로 뺐던거 다시 더해줘야함
                                        add_user(date_data, user, branch_data, space_data, start, end, people);
                                        add_date(date_data, branch_data, space_data, start, end);
                                    }
                                    return true;
        ...
        private boolean checkDatePassed(int year, int month, int day) { //YY MM DD 입력
